(function (global, factory) {
    ...
    function removeVNodeDOM(vNode, dom) {
        var flags = vNode.flags;
        if (flags & 2033 /* DOMRef */) {
            removeChild(dom, vNode.dom);
        }
        else {
    ...
    function directClone(vNodeToClone) {
        ...
        if (flags & 14 /* Component */) {
            ...
            newVNode = createFragment(vNodeToClone.children, vNodeToClone.childFlags, vNodeToClone.key);
        }
        return newVNode;
    }
    ...
    function replaceOneWithMany(lastChildren, nextChildren, parentDOM, context, isSVG) {
        var oldDOM = findDOMfromVNode(lastChildren);
                ...
                if (moved) {
                    ...
                    for (i = bLeft - 1; i >= 0; i--) {
                        if (sources[i] === 0) {
                            ...
                            nextPos = pos + 1;
                            nextNode = nextPos < bLength ? findDOMfromVNode(b[nextPos]) : outerEdge;
                            if (bNode.flags & 8192 /* Fragment */ && bNode.childFlags & 12 /* MultipleChildren */) {
                                for (var k = 0, len = bNode.children.length; k < len; k++) {
                                    insertOrAppend(dom, findDOMfromVNode(bNode.children[k]), nextNode);
                                }
                            }
                            else {
    ...
    var QUEUE = [];
    var nextTick = isFunction(Promise) ? Promise.resolve().then.bind(Promise.resolve()) : setTimeout;
