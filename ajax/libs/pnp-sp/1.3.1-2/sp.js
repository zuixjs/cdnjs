 ...
 */
import { stringIsNullOrEmpty, extend, combine, hOP, RuntimeConfig, FetchClient, mergeHeaders, getCtxCallback, isUrlAbsolute, mergeOptions, getGUID, jsS, objectDefinedNotNull, dateAdd, getHashCode, isArray } from '@pnp/common';
...
class SPHttpClient {
    ...
    fetch(url, options = {}) {
        ...
        if (!headers.has("X-ClientService-ClientTag")) {
            headers.append("X-ClientService-ClientTag", "PnPCoreJS:@pnp-1.3.1-2");
        }
        if (!headers.has("User-Agent")) {
            // this marks the requests for understanding by the service
            headers.append("User-Agent", "NONISV|SharePointPnP|PnPCoreJS/1.3.1-2");
        }
...
class ClientSidePage extends SharePointQueryable {
    ...
    save(publish = true) {
        ...
        promise = promise.then(_ => (ClientSidePage.initFrom(this, `_api/sitepages/pages(${this.json.Id})/savepage`)).postCore({
            body: jsS(Object.assign(metadata("SP.Publishing.SitePage"), {
...
class CanvasSection {
    constructor(page, order, columns = [], _emphasis = 0) {
        this.page = page;
