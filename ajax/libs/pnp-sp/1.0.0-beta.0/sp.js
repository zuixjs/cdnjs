...
class SharePointQueryable extends ODataQueryable {
     ...
     */
    toRequestContext(verb, options = {}, parser, pipeline = PipelineMethods.default) {
...
class SharePointQueryableSecurable extends SharePointQueryableInstance {
    ...
    getCurrentUserEffectivePermissions() {
        ...
        return w.currentUser.select("LoginName").getAs().then(user => {
...
class SharePointQueryableShareable extends SharePointQueryable {
    ...
    shareWith(loginNames, role, requireSignin = false, propagateAcl = false, emailData) {
        ...
        return Web.fromUrl(this.toUrl()).roleDefinitions.select("Id").filter(`RoleTypeKind eq ${roleFilter}`).get().then((def) => {
            ...
            return this.clone(SharePointQueryableShareable, "shareObject").postAsCore({
                body: JSON.stringify(postBody),
            });
        });
    }
        ...
        if (typeof group !== "undefined" && group !== null) {
            switch (group) {
                case RoleType.Contributor:
                    return Web.fromUrl(this.toUrl()).associatedMemberGroup.select("Id").getAs().then(g => `group: ${g.Id}`);
                case RoleType.Reader:
                case RoleType.Guest:
                    return Web.fromUrl(this.toUrl()).associatedVisitorGroup.select("Id").getAs().then(g => `group: ${g.Id}`);
                default:
            ...
            const roleFilter = role === SharingRole.Edit ? RoleType.Contributor : RoleType.Reader;
            return Web.fromUrl(this.toUrl()).roleDefinitions.select("Id").top(1).filter(`RoleTypeKind eq ${roleFilter}`).getAs().then(def => {
                if (def.length < 1) {
...
class SharePointQueryableShareableWeb extends SharePointQueryableSecurable {
    ...
    shareWith(loginNames, role = SharingRole.View, emailData) {
        ...
        return Web.fromUrl(this.toUrl(), "/_api/web/url").get().then((url) => {
            dependency();
...
class ContentTypes extends SharePointQueryableCollection {
    ...
    addAvailableContentType(contentTypeId) {
        ...
        return this.clone(ContentTypes, "addAvailableContentType").postAsCore({ body: postBody }).then((data) => {
            return {
...
class Views extends SharePointQueryableCollection {
    ...
    add(title, personalView = false, additionalSettings = {}) {
        ...
        return this.clone(Views, null).postAsCore({ body: postBody }).then((data) => {
            return {
                data: data,
...
class Fields extends SharePointQueryableCollection {
    ...
    createFieldAsXml(xml) {
        ...
        return this.clone(Fields, "createfieldasxml").postAsCore({ body: postBody }).then((data) => {
            return {
                data: data,
    ...
    add(title, fieldType, properties = {}) {
        ...
        return this.clone(Fields, null).postAsCore({ body: postBody }).then((data) => {
            return {
                data: data,
                field: this.getById(data.Id),
            };
...
class List extends SharePointQueryableSecurable {
    ...
    getListItemEntityTypeFullName() {
        return this.clone(List, null, false).select("ListItemEntityTypeFullName").getAs().then(o => o.ListItemEntityTypeFullName);
    }
}
...
class Items extends SharePointQueryableCollection {
    ...
    add(properties = {}, listItemEntityTypeFullName = null) {
        ...
        return this.ensureListItemEntityTypeName(listItemEntityTypeFullName).then(listItemEntityType => {
            ...
            const promise = this.clone(Items, null).postAsCore({ body: postBody }).then((data) => {
                return {
                    data: data,
...
class File extends SharePointQueryableShareableFile {
    ...
    startUpload(uploadId, fragment) {
        return this.clone(File, `startUpload(uploadId=guid'${uploadId}')`, false).postAsCore({ body: fragment }).then(n => parseFloat(n));
    }
    /**
    ...
    continueUpload(uploadId, fileOffset, fragment) {
        return this.clone(File, `continueUpload(uploadId=guid'${uploadId}',fileOffset=${fileOffset})`, false).postAsCore({ body: fragment }).then(n => parseFloat(n));
    }
    /**
    ...
    finishUpload(uploadId, fileOffset, fragment) {
        return this.clone(File, `finishUpload(uploadId=guid'${uploadId}',fileOffset=${fileOffset})`, false)
            .postAsCore({ body: fragment }).then((response) => {
            return {
                data: response,
...
class UtilityMethod extends SharePointQueryable {
     ...
     * @param includeBatch If true this instance's batch will be added to the cloned instance
     */
    create(methodName, includeBatch) {
