 ...
 */
import { extend, RuntimeConfig, FetchClient, combinePaths, Dictionary, mergeHeaders, getCtxCallback, isUrlAbsolute, mergeOptions, getGUID, getAttrValueFromString, dateAdd, readBlobAsArrayBuffer } from '@pnp/common';
...
class SPODataEntityParserImpl extends ODataParserBase {
    constructor(factory) {
        ...
        this.hydrate = (d) => {
            const o = new this.factory(spExtractODataId$1(d), null);
            return extend(o, d);
        };
    }
    parse(r) {
...
class SPODataEntityArrayParserImpl extends ODataParserBase {
    constructor(factory) {
        ...
        this.hydrate = (d) => {
            return d.map(v => {
                const o = new this.factory(spExtractODataId$1(v), null);
                return extend(o, v);
            });
        };
    }
...
class FileFolderShared extends SharePointQueryableInstance {
    ...
    getShareable() {
        ...
        return this.clone(SharePointQueryableShareableFile, "listItemAllFields", false).select("odata.id").get().then(d => {
            let shareable = new SharePointQueryableShareable(spExtractODataId$1(d));
