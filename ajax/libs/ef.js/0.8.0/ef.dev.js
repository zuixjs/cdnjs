(function (global, factory) {
	...
	var DOMARR = {
		...
		push: function push(ref) {
			...
			else { DOM.after.apply(DOM, [ this[this.length - 1].$ctx.placeholder ].concat( elements )); }
			exec();
	...
	var updateMountingNode = function (ref) {
		...
		if (value) {
			if (value.$ctx.nodeInfo.parent && 'development' !== 'production') { console.warn('[EF]', 'Better detach the component before attaching it to a new component!'); }
