...
var SyncPriority = 2;
var InteractivePriority = 3;
var DeferredPriority = 4;
...
function insertUpdateIntoQueue(queue, update) {
  ...
  if (priorityLevel !== NoPriority && queue.expirationTime <= update.expirationTime) {
    var node = queue.first;
    var latestExpirationTimeWithMatchingPriority = NoWork;
    while (node !== null) {
      if (node.priorityLevel === priorityLevel && node.expirationTime > latestExpirationTimeWithMatchingPriority) {
...
var ReactFiberClassComponent = function (scheduleWork, computeUpdatePriorityForFiber, memoizeProps, memoizeState, recalculateCurrentTime, computeExpirationTimeForPriority) {
  ...
  var updater = {
    ...
    enqueueSetState: function (instance, partialState, callback) {
      ...
      var priorityLevel = computeUpdatePriorityForFiber(fiber);
      var currentTime = recalculateCurrentTime();
      var expirationTime = computeExpirationTimeForPriority(priorityLevel, currentTime);
      var update = {
...
var ReactFiberBeginWork = function (config, hostContext, hydrationContext, scheduleWork, computeUpdatePriorityForFiber, recalculateCurrentTime, computeExpirationTimeForPriority) {
  ...
  var _ReactFiberClassCompo = ReactFiberClassComponent(scheduleWork, computeUpdatePriorityForFiber, memoizeProps, memoizeState, recalculateCurrentTime, computeExpirationTimeForPriority),
  ...
  function updateLoadingComponent(current, workInProgress, renderExpirationTime) {
    ...
    var prevState = current === null ? nextState : current.memoizedState;
  ...
  function updateTimeoutComponent(current, workInProgress, renderExpirationTime) {
    ...
    var updateQueue = workInProgress.updateQueue;
    if (updateQueue !== null) {
      nextState = workInProgress.memoizedState = processUpdateQueue(current, workInProgress, updateQueue, null, null, renderExpirationTime);
    }
    ...
    (workInProgress.effectTag & DidCapture) === NoEffect && prevProps === nextProps && prevState === nextState) {
      return bailoutOnAlreadyFinishedWork(current, workInProgress);
    }
  ...
  function updateContextConsumer(current, workInProgress, renderExpirationTime) {
    ...
    if (typeof render !== 'function') {
      invariant(false, 'A context consumer was rendered with multiple children, or a child that isn\'t a function. A context consumer expects a single child that is a function. If you did pass a function, make sure there is no trailing or leading whitespace around it.');
    }
  ...
  function beginWork(current, workInProgress, renderExpirationTime) {
    ...
    switch (workInProgress.tag) {
      ...
      case ReturnComponent:
        ...
        return null;
      case LoadingComponent:
        return updateLoadingComponent(current, workInProgress, renderExpirationTime);
...
function addRenderPhasePendingWork(root, startTime, expirationTime) {
...
function findNextExpirationTimeToWorkOn(root) {
  ...
  var lastRenderPhaseTime = NoWork;
  var lastRetryTime = NoWork;
  var work = root.firstPendingWork;
  while (work !== null) {
    if (!work.isSuspended && (!work.isRenderPhaseWork || lastSuspendedTime === NoWork)) {
    ...
    if (work.shouldTryResuming) {
      ...
      if (work.isRenderPhaseWork && (lastRenderPhaseTime === NoWork || lastRenderPhaseTime < work.expirationTime)) {
...
var SuspendException = 1;
var SuspendAndLoadingException = 2;

function createRootExpirationError(sourceFiber, renderExpirationTime) {
  try {
    // TODO: Better error messages.
    !(renderExpirationTime !== Sync) ? invariant(false, 'A synchronous update was suspended, but no fallback UI was provided.') : void 0;
    invariant(false, 'An update was suspended for longer than the timeout, but no fallback UI was provided.');
  } catch (error) {
    return error;
  }
}

var ReactFiberUnwindWork = function (hostContext, retryOnPromiseResolution, scheduleWork, isAlreadyFailedLegacyErrorBoundary, markTimeout) {
  ...
  function waitForPromise(root, promise, suspendedTime) {
    promise.then(function () {
      return retryOnPromiseResolution(root, suspendedTime);
    });
  }

  function scheduleLoadingState(workInProgress, renderStartTime, renderExpirationTime) {
    var slightlyEarlierExpirationTime = renderExpirationTime - 1;
    var loadingUpdate = {
    ...
    scheduleWork(workInProgress, renderStartTime, slightlyEarlierExpirationTime);
    return false;
  }

  function throwException(returnFiber, sourceFiber, value, renderIsExpired, remainingTimeMs, elapsedMs, renderStartTime, renderExpirationTime) {
    ...
    if (value !== null && typeof value === 'object' && typeof value.then === 'function') {
      // This is a thenable.
      var typeOfException = SuspendAndLoadingException;
      var _workInProgress = returnFiber;
      do {
        switch (_workInProgress.tag) {
          case HostRoot:
            {
              var _root = _workInProgress.stateNode;
                ...
                case SuspendException:
                  {
                    if (!renderIsExpired) {
                      ...
                      var promise = value;
                      suspendPendingWork(_root, suspendedTime);
                      waitForPromise(_root, promise, suspendedTime);
                      return;
                    }
                    // The root expired, but no fallback was provided. Throw a
                    // helpful error.
                    value = createRootExpirationError(sourceFiber, renderExpirationTime);
                    break;
                  }
              }
              break;
          ...
          case TimeoutComponent:
            switch (typeOfException) {
              ...
              case SuspendException:
                {
                  var didExpire = _workInProgress.memoizedState;
                  var timeout = _workInProgress.pendingProps.ms;
                  // Check if the boundary should capture promises that threw.
                  var shouldCapture = void 0;
                  if (_workInProgress.effectTag & DidCapture) {
                  ...
                  if (shouldCapture) {
                    ...
                    ensureUpdateQueues(_workInProgress);
                    var updateQueue = _workInProgress.updateQueue;
                    var capturedValues = updateQueue.capturedValues;
              ...
              case SuspendAndLoadingException:
                {
                  ...
                  var isLoading = _workInProgress.memoizedState;
                  if (current !== null && !isLoading && !renderIsExpired) {
                    // Schedule loading update
                    scheduleLoadingState(_workInProgress, renderStartTime, renderExpirationTime);
      ...
      switch (workInProgress.tag) {
        case HostRoot:
          {
            ...
            var _updateQueue = workInProgress.updateQueue;
            _updateQueue.capturedValues = [errorInfo];
        ...
        case ClassComponent:
          ...
          if ((workInProgress.effectTag & DidCapture) === NoEffect && (typeof ctor.getDerivedStateFromCatch === 'function' && enableGetDerivedStateFromCatch || _instance !== null && typeof _instance.componentDidCatch === 'function' && !isAlreadyFailedLegacyErrorBoundary(_instance))) {
            ...
            var _capturedValues = _updateQueue2.capturedValues;
            if (_capturedValues === null) {
              _updateQueue2.capturedValues = [value];
            } else {
              _capturedValues.push(value);
            }
            workInProgress.effectTag |= ShouldCapture;
            return;
          }
  ...
  function unwindWork(workInProgress, elapsedMs, renderIsExpired, remainingTimeMs, renderStartTime, renderExpirationTime) {
    switch (workInProgress.tag) {
      case ClassComponent:
        {
          popContextProvider(workInProgress);
...
var ReactFiberCommitWork = function (config, captureError, scheduleWork, computeUpdatePriorityForFiber, markLegacyErrorBoundaryAsFailed, recalculateCurrentTime, computeExpirationTimeForPriority) {
      ...
      case TimeoutComponent:
        {
          ...
          if (_updateQueue2 !== null) {
            var promises = _updateQueue2.capturedValues;
            if (promises !== null) {
  ...
  function commitErrorLogging(finishedWork, onUncaughtError) {
    switch (finishedWork.tag) {
      ...
      case HostRoot:
        {
          var _updateQueue3 = finishedWork.updateQueue;
          !(_updateQueue3 !== null && _updateQueue3.capturedValues !== null) ? invariant(false, 'An error logging effect should not have been scheduled if no errors were captured. This error is likely caused by a bug in React. Please file an issue.') : void 0;
          var _capturedErrors = _updateQueue3.capturedValues;
          _updateQueue3.capturedValues = null;
...
var ReactFiberScheduler = function (config) {
  ...
  var _ReactFiberBeginWork = ReactFiberBeginWork(config, hostContext, hydrationContext, scheduleWork, computeUpdatePriorityForFiber, recalculateCurrentTime, computeExpirationTimeForPriority),
      beginWork = _ReactFiberBeginWork.beginWork;

  var _ReactFiberCompleteWo = ReactFiberCompleteWork(config, hostContext, hydrationContext),
      completeWork = _ReactFiberCompleteWo.completeWork;

  var _ReactFiberUnwindWork = ReactFiberUnwindWork(hostContext, retryOnPromiseResolution, scheduleWork, isAlreadyFailedLegacyErrorBoundary, markTimeout),
      throwException = _ReactFiberUnwindWork.throwException,
      unwindWork = _ReactFiberUnwindWork.unwindWork;

  var _ReactFiberCommitWork = ReactFiberCommitWork(config, onCommitPhaseError, scheduleWork, computeUpdatePriorityForFiber, markLegacyErrorBoundaryAsFailed, recalculateCurrentTime, computeExpirationTimeForPriority),
  ...
  var priorityContext = NoPriority;
  ...
  if (true && replayFailedUnitOfWorkWithInvokeGuardedCallback) {
    ...
    replayUnitOfWork = function (thrownValue, failedUnitOfWork, isAsync) {
  ...
  function renderRoot(root, expirationTime, isAsync) {
    ...
    do {
      try {
        ...
        throwException(returnFiber, sourceFiber, thrownValue, nextRenderIsExpired, nextRemainingTimeMs, nextElapsedTimeMs, nextStartTime, nextRenderExpirationTime);
    ...
    if (didFatal) {
      ...
      if (isRootReadyForCommit) {
        ...
        if (nextEarliestTimeoutMs >= 0) {
          var ms = nextStartTimeMs + nextEarliestTimeoutMs - mostRecentCurrentTimeMs;
  ...
  function scheduleCapture(sourceFiber, boundaryFiber, value, priorityLevel, startTime, expirationTime) {
  ...
  function dispatch(sourceFiber, value, priorityLevel, startTime, expirationTime) {
      ...
      switch (fiber.tag) {
        case ClassComponent:
          ...
          if (typeof ctor.getDerivedStateFromCatch === 'function' || typeof instance.componentDidCatch === 'function' && !isAlreadyFailedLegacyErrorBoundary(instance)) {
            scheduleCapture(sourceFiber, fiber, value, priorityLevel, startTime, expirationTime);
            return;
          }
          break;
    ...
    if (sourceFiber.tag === HostRoot) {
      ...
      scheduleCapture(sourceFiber, sourceFiber, value, priorityLevel, startTime, expirationTime);
    }
  }
  ...
  function computeUpdatePriorityForFiber(fiber) {
  ...
  function computeExpirationTimeForPriority(priorityLevel, startTime) {
  ...
  function retryOnPromiseResolution(root, suspendedTime) {
  ...
  function markTimeout(timeoutMs) {
  ...
  return {
    ...
    computeUpdatePriorityForFiber: computeUpdatePriorityForFiber,
    computeExpirationTimeForPriority: computeExpirationTimeForPriority,
...
var ReactFiberReconciler$1 = function (config) {
  ...
  var _ReactFiberScheduler = ReactFiberScheduler(config),
      ...
      computeUpdatePriorityForFiber = _ReactFiberScheduler.computeUpdatePriorityForFiber,
      computeExpirationTimeForPriority = _ReactFiberScheduler.computeExpirationTimeForPriority,
  ...
  function updateContainerAtExpirationTime(element, container, parentComponent, currentTime, priorityLevel, expirationTime, callback) {
  ...
  return {
    ...
    updateContainer: function (element, container, parentComponent, callback) {
      var current = container.current;
      var priorityLevel = computeUpdatePriorityForFiber(current);
      var currentTime = recalculateCurrentTime();
      var expirationTime = computeExpirationTimeForPriority(priorityLevel, currentTime);
      return updateContainerAtExpirationTime(element, container, parentComponent, currentTime, priorityLevel, expirationTime, callback);
    },
