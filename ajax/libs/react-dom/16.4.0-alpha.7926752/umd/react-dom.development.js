...
var replayFailedBeginPhaseWithInvokeGuardedCallback = true;
...
var didWarnUpdateInsideUpdater = void 0;
var warnForUpdatesInsideUpdater = void 0;

{
  didWarnUpdateInsideUpdater = false;
  warnForUpdatesInsideUpdater = function (queue1, queue2) {
    if ((queue1.isProcessing || queue2 !== null && queue2.isProcessing) && !didWarnUpdateInsideUpdater) {
...
function ensureUpdateQueues(fiber) {
  ...
  if (q1 === null) {
    ...
    q1 = fiber.updateQueue = createUpdateQueue(null);
  }

  if (alternateFiber !== null) {
    q2 = alternateFiber.updateQueue;
    if (q2 === null) {
      q2 = alternateFiber.updateQueue = createUpdateQueue(null);
    }
  } else {
    q2 = null;
  }
...
function insertUpdateIntoFiber(fiber, update) {
  ...
  var expirationTime = update.expirationTime;
  var insertAfter1 = queue1.last;
  ...
  if (queue2.expirationTime === NoWork || queue2.expirationTime > update.expirationTime) {
    queue2.expirationTime = update.expirationTime;
  }
}

function insertUpdateIntoQueue(queue, update, insertAfter) {
  var insertBefore = insertAfter === null ? queue.first : insertAfter.next;
  if (insertAfter === null) {
    queue.first = update;
  } else {
...
function findRenderPhaseUpdateInsertionPosition(queue, expirationTime) {
...
function insertRenderPhaseUpdateIntoFiber(fiber, update) {
  ensureUpdateQueues(fiber);
  // Uses module scope to avoid allocating a tuple
  var queue1 = q1;
  var queue2 = q2;
  ...
  var expirationTime = update.expirationTime;
  var insertAfter1 = findRenderPhaseUpdateInsertionPosition(queue1, expirationTime);
  ...
  // We must calculate the second insertion position *before* performing
  // any insertions.
  var insertAfter2 = findRenderPhaseUpdateInsertionPosition(queue2, expirationTime);
...
var ReactFiberClassComponent = function (scheduleWork, computeExpirationForFiber, memoizeProps, memoizeState, recalculateCurrentTime, checkIfInRenderPhase) {
  ...
  function checkClassInstance(workInProgress) {
    ...
    if (typeof instance.getChildContext === 'function') {
      warning_1(typeof type.childContextTypes === 'object', '%s.getChildContext(): childContextTypes must be defined in order to ' + 'use getChildContext().', getComponentName(workInProgress));
    }
  }
    ...
    if (typeof type.getDerivedStateFromProps === 'function') {
      {
        ...
        if (typeof instance.componentWillReceiveProps === 'function' && instance.componentWillReceiveProps.__suppressDeprecationWarning !== true || typeof instance.UNSAFE_componentWillReceiveProps === 'function') {
          ...
          if (!didWarnAboutWillReceivePropsAndDerivedState[componentName]) {
            warning_1(false, '%s: Defines both componentWillReceiveProps() and static ' + 'getDerivedStateFromProps() methods. We recommend using ' + 'only getDerivedStateFromProps().', componentName);
...
function ChildReconciler(shouldTrackSideEffects) {
  ...
  function reconcileChildFibers(returnFiber, currentFirstChild, newChild, deleteExistingChildren, expirationTime) {
...
function popProvider(providerFiber) {
  {
    warning_1(index$1 > -1 && providerFiber === stack[index$1], 'Unexpected pop.');
  }
...
function createCapturedValue(value, tag, source) {
...
function addPendingWork(root, startTime, expirationTime) {
  var match = null;
  var insertAfter = null;
  var insertBefore = root.firstPendingWork;
  while (insertBefore !== null) {
    if (insertBefore.expirationTime >= expirationTime) {
...
function addNonInteractivePendingWork(root, startTime, expirationTime) {
...
function flushPendingWork(root, currentTime, remainingExpirationTime) {
  // Pop all work that has higher priority than the remaining priority.
  var firstUnflushedWork = root.firstPendingWork;
  while (firstUnflushedWork !== null) {
    if (remainingExpirationTime !== NoWork && firstUnflushedWork.expirationTime >= remainingExpirationTime) {
      break;
    }
  ...
  } else if (remainingExpirationTime !== NoWork && firstUnflushedWork.expirationTime > remainingExpirationTime) {
    // There was an update during the render phase that wasn't flushed.
    addNonInteractivePendingWork(root, currentTime, remainingExpirationTime);
  }
}

function suspendPendingWork(root, expirationTime) {
  var work = root.firstPendingWork;
  while (work !== null) {
...
function resumePendingWork(root, expirationTime) {
...
function findNextExpirationTimeToWorkOn(root) {
  ...
  var lastSuspendedTime = NoWork;
  var lastNonInteractiveTime = NoWork;
  var lastRetryTime = NoWork;
  var work = root.firstPendingWork;
  while (work !== null) {
    if (!work.isSuspended && (work.isInteractive || lastSuspendedTime === NoWork)) {
    ...
    if (work.shouldTryResuming) {
      ...
      if (!work.isInteractive && (lastNonInteractiveTime === NoWork || lastNonInteractiveTime < work.expirationTime)) {
...
function findStartTime(root, expirationTime) {
  var match = root.firstPendingWork;
  while (match !== null) {
...
function raiseUnknownEffect(returnFiber, sourceFiber, value) {
  var algebraicEffectTag = void 0;
  if (value instanceof Promise) {
    ...
    if (typeof value.then === 'function') {
      algebraicEffectTag = Suspend;
    } else if (typeof value.stack === 'string' && typeof value.message === 'string') {
      algebraicEffectTag = Err;
    } else {
...
function raiseCombinedEffect(returnFiber, sourceFiber, values) {
...
function createRootExpirationError(sourceFiber, renderExpirationTime) {
  try {
    ...
    !(renderExpirationTime !== Sync) ? invariant_1(false, 'A synchronous update was suspended, but no fallback UI was provided.') : void 0;
    invariant_1(false, 'An update was suspended for longer than the timeout, but no fallback UI was provided.');
  } catch (error) {
    return createCapturedValue(error, Err, sourceFiber);
  }
}

var ReactFiberIncompleteWork = function (hostContext, retryOnPromiseResolution, scheduleWork, markUncaughtError, isAlreadyFailedLegacyErrorBoundary) {
  var popHostContainer = hostContext.popHostContainer,
      popHostContext = hostContext.popHostContext;


  function handleRootLoading(current, workInProgress, promises, boundary, suspenders, renderIsExpired, renderStartTime, renderExpirationTime) {
    if (!renderIsExpired) {
      var slightlyHigherPriority = renderExpirationTime - 1;
    ...
    var errorInfo = createRootExpirationError(boundary, renderExpirationTime);
    return handleRootError(current, workInProgress, errorInfo, renderExpirationTime);
  }

  function handleRootSuspend(current, workInProgress, promises, sourceFiber, promise, renderIsExpired, renderExpirationTime) {
    if (!renderIsExpired) {
      promises.push(promise);
      return false;
    }
    var errorInfo = createRootExpirationError(sourceFiber, renderExpirationTime);
    return handleRootError(current, workInProgress, errorInfo, renderExpirationTime);
  }
  ...
  function handleRootUnknown(current, workInProgress, sourceFiber, unknownValue, renderExpirationTime) {
    // Turn this into an error info object
    unknownValue.tag = Err;
    return handleRootError(current, workInProgress, unknownValue, renderExpirationTime);
  }

  function handleRootError(current, workInProgress, errorInfo, renderExpirationTime) {
  ...
  function handleRootEffect(current, workInProgress, promises, earliestTimeoutRef, effectSource, algebraicEffectTag, effectValue, renderIsExpired, renderStartTime, renderExpirationTime) {
    switch (algebraicEffectTag) {
      case Combined:
        {
          var nestedValues = effectValue;
          var shouldRetry = false;
          for (var i = 0; i < nestedValues.length; i++) {
            var nestedValue = nestedValues[i];
            if (handleRootEffect(current, workInProgress, promises, earliestTimeoutRef, nestedValue.source, nestedValue.tag, nestedValue, renderIsExpired, renderStartTime, renderExpirationTime)) {
              shouldRetry = true;
            }
          }
          return shouldRetry;
      ...
      case Loading:
        {
          return handleRootLoading(current, workInProgress, promises, effectSource, effectValue, renderIsExpired, renderStartTime, renderExpirationTime);
        }
      case Suspend:
        {
          return handleRootSuspend(current, workInProgress, promises, effectSource, effectValue, renderIsExpired, renderExpirationTime);
        }
      case Timeout:
        {
      ...
      case NoEffect:
        ...
        return handleRootUnknown(current, workInProgress, effectSource, effectValue, renderExpirationTime);
      case Err:
        return handleRootError(current, workInProgress, effectValue, renderExpirationTime);
      default:
        invariant_1(false, 'Cannot have more than one algebraic effect tag. They should be mutally exclusive. This error is likely caused by a bug in React. Please file an issue.');
    }
  }

  function exitIncompleteRoot(workInProgress, renderIsExpired, remainingTimeMs, renderStartTime, renderExpirationTime) {
    var root = workInProgress.stateNode;
    var current = root.current;

    var promises = [];
    var earliestTimeoutRef = { value: remainingTimeMs };
    var shouldRetry = void 0;
    var effect = workInProgress.firstEffect;
    while (effect !== null) {
      var effectValue = effect.thrownValue;
      var effectSource = effect;
      var algebraicEffectTag = effect.effectTag & AlgebraicEffectMask;
      shouldRetry = handleRootEffect(current, workInProgress, promises, earliestTimeoutRef, effectSource, algebraicEffectTag, effectValue, renderIsExpired, renderStartTime, renderExpirationTime);
  ...
  function handleClassEffect(workInProgress, errors, algebraicEffectTag, effectValue) {
    switch (algebraicEffectTag) {
      case Err:
        {
          var errorInfo = effectValue;
  ...
  function exitIncompleteWork(workInProgress, elapsedMs, renderIsExpired, remainingTimeMs, renderStartTime, renderExpirationTime) {
    var current = workInProgress.alternate;
    ...
    switch (workInProgress.tag) {
      ...
      case ClassComponent:
        {
          ...
          if ((workInProgress.effectTag & DidCapture) === NoEffect && (typeof ctor.getDerivedStateFromCatch === 'function' && enableGetDerivedStateFromCatch || _instance !== null && typeof _instance.componentDidCatch === 'function' && !isAlreadyFailedLegacyErrorBoundary(_instance))) {
            ...
            while (effect !== null) {
              var algebraicEffectTag = effect.effectTag & AlgebraicEffectMask;
              var effectValue = effect.thrownValue;
              var didHandle = handleClassEffect(workInProgress, errors, algebraicEffectTag, effectValue);
              if (didHandle) {
      ...
      case HostRoot:
        ...
        next = exitIncompleteRoot(workInProgress, renderIsExpired, remainingTimeMs, renderStartTime, renderExpirationTime);
        break;
      case HostComponent:
      ...
      case ReturnComponent:
        break;
      case AsyncBoundary:
        var isLoading = workInProgress.memoizedState;
        if (current !== null && !isLoading && !renderIsExpired) {
          var promises = null;
          var _previousEffect = null;
          var _effect = workInProgress.firstEffect;
          while (_effect !== null) {
            var thrownValue = _effect.thrownValue;
            var _algebraicEffectTag = _effect.effectTag & AlgebraicEffectMask;
            switch (_algebraicEffectTag) {
              case Suspend:
                captureEffect(workInProgress, _previousEffect, _effect);
                if (promises === null) {
            ...
            _previousEffect = _effect;
            _effect = _effect.nextEffect;
          }
          if (promises !== null) {
      ...
      case TimeoutComponent:
        {
          ...
          if (_updateQueue !== null) {
            nextState = workInProgress.memoizedState = processUpdateQueue(current, workInProgress, _updateQueue, null, null, renderExpirationTime);
          }

          var didExpire = nextState !== null;
          var didCaptureAlready = workInProgress.effectTag & DidCapture;

          var timeout = workInProgress.pendingProps.ms;

          // Check if the boundary should capture promises that threw.
          var shouldCapture = void 0;
          ...
          if (shouldCapture) {
            ...
            var _promises = null;
            var _previousEffect2 = null;
            var _effect2 = workInProgress.firstEffect;
            while (_effect2 !== null) {
              var _algebraicEffectTag2 = _effect2.effectTag & AlgebraicEffectMask;
              switch (_algebraicEffectTag2) {
                case Suspend:
                case Loading:
                  var promise = _effect2.thrownValue;
                  if (_promises === null) {
                    _promises = [promise];
                  } else {
                  ...
                  _promises.push(promise);
                  captureEffect(workInProgress, _previousEffect2, _effect2);
                  break;
                default:
                  break;
              }
              _previousEffect2 = _effect2;
              _effect2 = _effect2.nextEffect;
            }
            if (_promises !== null) {
              // Store the promises. We'll use these in the commit phase to
              // schedule a recovery effect.
              workInProgress.memoizedState = _promises;
              workInProgress.effectTag |= DidCapture;
              // Render the boundary again
              next = workInProgress;
            }
          ...
          if (next === null && typeof timeout === 'number') {
            var _returnFiber = workInProgress['return'];
            if (_returnFiber !== null) {
              raiseTimeoutEffect(_returnFiber, workInProgress, timeout);
            }
          }
          break;
        }
  ...
  return {
    exitIncompleteWork: exitIncompleteWork
  };
};
...
var ReactFiberBeginWork = function (config, hostContext, hydrationContext, scheduleWork, computeExpirationForFiber, recalculateCurrentTime, checkIfInRenderPhase) {
  ...
  var _ReactFiberClassCompo = ReactFiberClassComponent(scheduleWork, computeExpirationForFiber, memoizeProps, memoizeState, recalculateCurrentTime, checkIfInRenderPhase),
  ...
  function reconcileChildren(current, workInProgress, nextChildren) {
    reconcileChildrenAtExpirationTime(current, workInProgress, nextChildren, false, workInProgress.expirationTime);
  }

  function reconcileChildrenAtExpirationTime(current, workInProgress, nextChildren, deleteExistingChildren, renderExpirationTime) {
    if (current === null) {
      // If this is a fresh new component that hasn't been rendered yet, we
      // won't update its child set by applying minimal side-effects. Instead,
      // we will add them all to the child before it gets rendered. That means
      // we can optimize this reconciliation pass by not tracking side-effects.
      workInProgress.child = mountChildFibers(workInProgress, null, nextChildren, deleteExistingChildren, renderExpirationTime);
    } else {
      ...
      // let's throw it out.
      workInProgress.child = reconcileChildFibers(workInProgress, current.child, nextChildren, deleteExistingChildren, renderExpirationTime);
    }
  }
  ...
  function finishClassComponent(current, workInProgress, shouldUpdate, hasContext, didCaptureError, renderExpirationTime) {
    ...
    // React DevTools reads this flag.
    workInProgress.effectTag |= PerformedWork;
    reconcileChildrenAtExpirationTime(current, workInProgress, nextChildren, didCaptureError, renderExpirationTime);
  ...
  function updateHostRoot(current, workInProgress, renderExpirationTime) {
    ...
    if (updateQueue !== null) {
      ...
      if ((current === null || current.child === null) && root.hydrate && enterHydrationState(workInProgress)) {
        ...
        workInProgress.child = mountChildFibers(workInProgress, null, element, false, renderExpirationTime);
      } else {
  ...
  function updateCallComponent(current, workInProgress, renderExpirationTime) {
    ...
    if (current === null) {
      workInProgress.stateNode = mountChildFibers(workInProgress, workInProgress.stateNode, nextChildren, false, renderExpirationTime);
    } else {
      workInProgress.stateNode = reconcileChildFibers(workInProgress, current.stateNode, nextChildren, false, renderExpirationTime);
    }
  ...
  function updateAsyncBoundary(current, workInProgress, renderExpirationTime) {
    ...
    var updateQueue = workInProgress.updateQueue;
    if (updateQueue !== null) {
      nextState = workInProgress.memoizedState = processUpdateQueue(current, workInProgress, updateQueue, null, nextProps, renderExpirationTime);
    }

    var isLoading = nextState;
    if (hasContextChanged()) {
      // Normally we can bail out on props equality but if context has changed
      // we don't do the bailout and we have to reuse existing props instead.
    ...
    var nextChildren = render(isLoading);
    workInProgress.memoizedProps = nextProps;
    reconcileChildren(current, workInProgress, nextChildren);
    return workInProgress.child;
  }

  function updateTimeoutComponent(current, workInProgress, capturedValues, renderExpirationTime) {
    var nextProps = workInProgress.pendingProps;
    var render = nextProps.children;

    // Check whether we already attempted to render this boundary during
    // this render.
    var isExpired = (workInProgress.effectTag & DidCapture) !== NoEffect;

    var nextChildren = render(isExpired);
    workInProgress.memoizedProps = nextProps;
    reconcileChildren(current, workInProgress, nextChildren);
    return workInProgress.child;
  }
  ...
  function updatePortalComponent(current, workInProgress, renderExpirationTime) {
    ...
    if (current === null) {
      ...
      workInProgress.child = reconcileChildFibers(workInProgress, null, nextChildren, false, renderExpirationTime);
      memoizeProps(workInProgress, nextChildren);
    } else {
      reconcileChildren(current, workInProgress, nextChildren);
      memoizeProps(workInProgress, nextChildren);
  ...
  function beginWork(current, workInProgress, renderExpirationTime) {
    ...
    switch (workInProgress.tag) {
      ...
      case ReturnComponent:
        ...
        return null;
      case AsyncBoundary:
        return updateAsyncBoundary(current, workInProgress, renderExpirationTime);
      case TimeoutComponent:
...
var ReactFiberCompleteWork = function (config, hostContext, hydrationContext) {
  ...
  function moveCallToHandlerPhase(current, workInProgress, renderExpirationTime) {
    ...
    workInProgress.child = reconcileChildFibers(workInProgress, currentFirstChild, nextChildren, false, renderExpirationTime);
    return workInProgress.child;
  }
...
var ReactFiberCommitWork = function (config, captureError, scheduleWork, computeExpirationForFiber, markLegacyErrorBoundaryAsFailed, recalculateCurrentTime) {
  ...
  function commitLifeCycles(finishedRoot, current, finishedWork, currentTime, committedExpirationTime) {
    switch (finishedWork.tag) {
      ...
      case TimeoutComponent:
        {
          var promises = finishedWork.memoizedState;
          Promise.all(promises).then(function () {
            return scheduleExpirationBoundaryRecovery(finishedWork);
          });
          return;
        }
      default:
  ...
  function commitErrorLogging(finishedWork) {
    switch (finishedWork.tag) {
      case ClassComponent:
        {
...
var ReactFiberScheduler = function (config) {
  ...
  var _ReactFiberBeginWork = ReactFiberBeginWork(config, hostContext, hydrationContext, scheduleWork, computeExpirationForFiber, recalculateCurrentTime, checkIfInRenderPhase),
      beginWork = _ReactFiberBeginWork.beginWork;

  var _ReactFiberCompleteWo = ReactFiberCompleteWork(config, hostContext, hydrationContext),
      completeWork = _ReactFiberCompleteWo.completeWork;

  var _ReactFiberIncomplete = ReactFiberIncompleteWork(hostContext, retryOnPromiseResolution, scheduleWork, markUncaughtError, isAlreadyFailedLegacyErrorBoundary),
      exitIncompleteWork = _ReactFiberIncomplete.exitIncompleteWork;
  ...
  var isWorking = false;
  var updatesAreInteractive = false;
  ...
  var nextStartTime = NoWork;
  var nextStartTimeMs = -1;
  var nextElapsedTimeMs = -1;
  var nextRemainingTimeMs = -1;
  var nextRenderCursor = NoWork;
  var nextRenderIsExpired = false;

  // The next fiber with an effect that we're currently committing.
  var nextEffect = null;

  var didThrowUncaughtError = false;
  ...
  function commitRoot(finishedWork) {
    ...
    var remainingTime = findNextExpirationTimeToWorkOn(root);
  ...
  function unwindUnitOfWork(workInProgress) {
    ...
    while (true) {
      ...
      if ((workInProgress.effectTag & Incomplete) === NoEffect) {
        ...
        var _next = exitIncompleteWork(workInProgress, nextElapsedTimeMs, nextRenderIsExpired, nextRemainingTimeMs, nextStartTime, nextRenderExpirationTime);
  ...
  var invokeBeginWork = beginWork;
  if (true && replayFailedBeginPhaseWithInvokeGuardedCallback) {
    invokeBeginWork = function beginWorkInDEV(current, workInProgress, expirationTime) {
      var stashedWorkInProgressProperties = _assign({}, workInProgress);
      try {
        return beginWork(current, workInProgress, expirationTime);
      } catch (value) {
        var thrownValue = value;
        ...
        if (value instanceof Error || typeof value === 'object' && value !== null && typeof value.message === 'string' && typeof value.stack === 'string') {
          // Retore the original state of the work-in-progress
          _assign(workInProgress, stashedWorkInProgressProperties);
          ...
          invokeGuardedCallback$2(null, beginWork, null, current, workInProgress, expirationTime);
          if (hasCaughtError()) {
  ...
  function performUnitOfWork(workInProgress) {
    ...
    var next = invokeBeginWork(current, workInProgress, nextRenderExpirationTime);
  ...
  function renderRoot(root, renderCursor, isAsync) {
    ...
    if (renderCursor !== nextRenderCursor || root !== nextRoot || nextUnitOfWork === null) {
      ...
      nextRemainingTimeMs = expirationTimeToMs(nextRenderExpirationTime) - mostRecentCurrentTimeMs;
    ...
    if (didThrowUncaughtError) {
      ...
      var finishedWork = isRootReadyForCommit ? root.current.alternate : null;
      ...
      if (isRootReadyForCommit) {
        ...
        root.pendingCommitExpirationTime = renderCursor;
        var _finishedWork = root.current.alternate;
  ...
  function scheduleCapture(sourceFiber, boundaryFiber, value, startTime, expirationTime) {
    // TODO: We only support dispatching errors.
    var algebraicEffectTag = Err;
    var capturedValue = createCapturedValue(value, algebraicEffectTag, sourceFiber);
    var update = {
      expirationTime: expirationTime,
      partialState: null,
      callback: null,
  ...
  function dispatch(sourceFiber, value, startTime, expirationTime) {
    ...
    if (sourceFiber.tag === HostRoot) {
      ...
      scheduleCapture(sourceFiber, sourceFiber, value, startTime, expirationTime);
    }
  }

  function onCommitPhaseError(fiber, error) {
    var startTime = recalculateCurrentTime();
  ...
  function retryOnPromiseResolution(root, blockedTime) {
    resumePendingWork(root, blockedTime);
    var retryTime = findNextExpirationTimeToWorkOn(root);
  ...
  function scheduleWork(fiber, startTime, expirationTime) {
    return scheduleWorkImpl(fiber, startTime, expirationTime, false);
  }

  function scheduleWorkImpl(fiber, startTime, expirationTime, isErrorRecovery) {
  ...
  function deferredUpdates(fn) {
    ...
    var previousUpdatesAreInteractive = updatesAreInteractive;
    var currentTime = recalculateCurrentTime();
  ...
  function performWorkOnRoot(root, expirationTime, isAsync) {
    ...
    if (!isAsync) {
      ...
      // Flush async work.
      var _finishedWork2 = root.finishedWork;
      if (_finishedWork2 !== null) {
        // This root is already complete. We can commit it.
        completeRoot(root, _finishedWork2, expirationTime);
      } else {
        root.finishedWork = null;
        _finishedWork2 = renderRoot(root, expirationTime, true);
        if (_finishedWork2 !== null) {
          ...
          if (!shouldYield()) {
            ...
            root.finishedWork = _finishedWork2;
          }
        }
      }
    }
