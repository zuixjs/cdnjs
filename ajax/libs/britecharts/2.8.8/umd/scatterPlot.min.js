...
!(__WEBPACK_AMD_DEFINE_RESULT__ = function () {
    ...
    var d3Shape = __webpack_require__(108);
    ...
    return function module() {
        ...
        */
        function drawDataPointsValueHighlights(data) {
            ...
            highlightCrossHairLabelsContainer.selectAll('text.highlight-y-legend').data([data]).enter().append('text').attr('text-anchor', 'middle').attr('fill', function (_ref9) {
                var name = _ref9.name;
                return nameColorMap[name];
            }).attr('class', 'highlight-y-legend').attr('y', function (d) {
                return yScale(d.y) + areaScale(d.y) / 2;
            }).attr('x', highlightTextLegendOffset).text(function (d) {
                return '' + d3Format.format(yAxisFormat)(d.y);
            });

            // Draw data label for x value
            highlightCrossHairLabelsContainer.selectAll('text.highlight-x-legend').data([data]).enter().append('text').attr('text-anchor', 'middle').attr('fill', function (_ref10) {
                var name = _ref10.name;
                return nameColorMap[name];
            }).attr('class', 'highlight-x-legend').attr('transform', 'translate(0, ' + (chartHeight - highlightTextLegendOffset) + ')').attr('x', function (d) {
                return xScale(d.x) - areaScale(d.y) / 2;
            }).text(function (d) {
                return '' + d3Format.format(xAxisFormat)(d.x);
            });
        }
        ...
        function handleClickAnimation(dataPoint) {
            highlightCircle.transition().ease(ease).duration(clickDuration).attr('r', function () {
                return areaScale(dataPoint.y * 2);
            }).transition().ease(ease).delay(clickDelay).duration(clickDuration).attr('r', function () {
                return areaScale(dataPoint.y);
            });
        }
        ...
        */
         ...
         * @private
         */
        function setCrossHairLinesStatus() {
