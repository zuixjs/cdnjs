...
(function (global, factory) {
  ...
  var isTextInput = function (el) { return ['text','password','search','email','tel','url',
      'textarea'].indexOf(el.type) !== -1; };
  var isCheckboxOrRadioInput = function (el) { return ['radio','checkbox'].indexOf(el.type) !== -1; };
  var isDateInput = function (el) { return ['date','week','month','datetime-local','time'].indexOf(el.type) !== -1; };
  ...
  var validate$8 = function (value, options) {
      ...
      return toArray(options).some(function (item) { return item == value; });
  };
  ...
  ErrorBag.prototype._makeCandidateFilters = function _makeCandidateFilters (selector) {
      ...
      var isPrimary = function (item) { return matchesName(item) && matchesRule(item) && matchesScope(item); };
      var isAlt = function (item) { return matchesRule(item) && item.field === (scope + "." + name); };
      return {
          isPrimary: isPrimary,
          isAlt: isAlt
      };
  ...
  Validator.prototype.validateAll = function validateAll (values, scope, silent) {
      ...
      return new Promise((function ($return, $error) {
          ...
          var matcher, providedValues, results;
          if (this.paused) 
              { return $return(true); }
          matcher = null;
  ...
  Validator.prototype._validate = function _validate (field, value) {
      return new Promise(function ($return, $error) {
          ...
          var promises, errors, isExitEarly;
