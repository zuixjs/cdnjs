...
var actionFieldDecorator = createClassPropertyDecorator(function (target, key, value, args, originalDescriptor) {
    ...
    return this[key];
}, dontReassignFields, false, true);
var boundActionDecorator = createClassPropertyDecorator(function (target, key, value) {
    defineBoundAction(target, key, value);
}, function (key) {
    return this[key];
}, dontReassignFields, false, false);
...
action.bound = function boundAction(arg1, arg2, arg3) {
    if (process.env.NODE_ENV !== "production" && typeof arg1 === "function") {
        return fail$1("'action.bound' cannot be used function anymore, use decorators instead");
    }
...
function when(predicate, effect, opts) {
    return autorun(function (r) {
        if (predicate()) {
            r.dispose();
...
function defineComputedProperty(adm, propName, options, asInstanceProperty) {
...
function setPropertyValue(instance, key, newValue) {
    var adm = instance.$mobx;
    var observable = adm.values[key];
...
function createComputedDecorator(options) {
    return createClassPropertyDecorator(function (target, name, _, __, originalDescriptor) {
        process.env.NODE_ENV !== "production" &&
            invariant(typeof originalDescriptor !== "undefined" &&
                typeof originalDescriptor.get === "function", "@computed can only be used on getter functions like: '@computed get myProps() { return ...; }'.");
        var adm = asObservableObject(target, "");
        var get = originalDescriptor.get, set = originalDescriptor.set;
        defineComputedProperty(adm, name, __assign({}, options, { get: get, set: set }), false);
    }, function (name) {
        var observable = this.$mobx.values[name];
        if (observable === undefined // See #505
        )
...
var computedStructDecorator = createComputedDecorator({ equals: comparer.structural });
/**
...
function extendObservable(target, properties, decorators, options) {
    ...
    // TODO:
    var defaultDecorator = options.defaultDecorator || (options.deep === false ? observable.ref : observable.deep);
    asObservableObject(target); // make sure it can be observable
    startBatch();
    try {
        var additionalDecorators = {}; // don't want to modify passed in object
        var unassigned = [];
        for (var key in properties) {
            var descriptor = Object.getOwnPropertyDescriptor(properties, key);
            var value = descriptor.value, get = descriptor.get;
            ...
            if (typeof get === "function") {
                ...
                if (!decorators || !decorators[key])
                    additionalDecorators[key] = computed;
            }
            else {
                unassigned.push(key);
                if (!decorators || !decorators[key])
        ...
        unassigned.forEach(function (key) { return (target[key] = properties[key]); }); // TODO: optimize
    }
    finally {
        endBatch();
    }
...
function remove(obj, key) {
    if (isObservableObject(obj)) {
        ...
        if (!adm.values[key])
            return;
        if (adm.keys) {
            var keyIndex = adm.keys.indexOf(key);
            if (keyIndex !== -1)
                adm.keys.splice(keyIndex, 1);
        }
...
exports._setReactionScheduler = setReactionScheduler;
exports.onReactionError = onReactionError;
exports._reserveArrayBuffer = reserveArrayBuffer;
