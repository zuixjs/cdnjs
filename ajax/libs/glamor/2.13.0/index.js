...
var styleSheet = exports.styleSheet = new _sheet.StyleSheet({ name: '_css_' });
...
// plugins 
var plugins = exports.plugins = styleSheet.plugins = new _plugins.PluginSet(_plugins.fallbacks, _plugins.prefixes);
...
function insertRule(css, id) {
  var hasId = !!id;
  id = id || (0, _hash2.default)(css).toString(36);
...
function selector(id, type) {
  ...
  var isParentSelector = type && type[0] === '%';
  var cssType = type === '_' ? '' : // plain style object
  ...
  if (canSimulate && type !== '_' && !isFullSelector && !isParentSelector && cssType[0] === ':') {
...
function cssrule(id, type, style) {
  var result = plugins.apply({ id: id, type: type, style: style, selector: selector(id, type) });
...
function add() {
  ...
  if (!styleSheet.cache[id]) {
    ...
    styleSheet.cache[id] = { type: type, style: style, id: id };
  }
  if (hasLabels) {
    // adds a debug label 
...
function placeholder(x) {
  ...
  return merge(add(':placeholder', x), add(':-webkit-input-placeholder', x), add(':-moz-placeholder', x), add(':-ms-input-placeholder', x));
}
...
function select(selector, _style) {
...
function merge() {
  ...
  rules.forEach(function (rule, i) {
    ...
    if (isRule(rule)) {
      ...
      if (styleSheet.cache[_id].bag) {
        var _ret = function () {
          // merged rule 

          var _styleSheet$cache$_id = styleSheet.cache[_id];
          var bag = _styleSheet$cache$_id.bag;
          var label = _styleSheet$cache$_id.label;
          var media = _styleSheet$cache$_id.media;
      ...
      if (styleSheet.cache[_id].expr) {
        var _ret2 = function () {
          // media rule
          var _styleSheet$cache$_id2 = styleSheet.cache[_id];
          var expr = _styleSheet$cache$_id2.expr;
          var label = _styleSheet$cache$_id2.label;
          var rule = _styleSheet$cache$_id2.rule;
          var style = _styleSheet$cache$_id2.style;
          ...
          if (rule) {
            ...
            if (styleSheet.cache[iid].bag) {
              (function () {
                ...
                var bag = styleSheet.cache[iid].bag;
              ...
              var _styleSheet$cache$iid = styleSheet.cache[iid];
              var type = _styleSheet$cache$iid.type;
              var _style2 = _styleSheet$cache$iid.style;

              mediaBag[expr][type] = _extends({}, mediaBag[expr][type] || {}, _style2);
            }
          } else {
        ...
        var _styleSheet$cache$_id3 = styleSheet.cache[_id];
        var type = _styleSheet$cache$_id3.type;
        var _style3 = _styleSheet$cache$_id3.style;

        styleBag[type] = _extends({}, styleBag[type] || {}, _style3);
        hasLabels && labels.push(_style3.label || '*'); // todo - match 'add()'s original label
        return;
        // not too bad 
      }
    } else {
  ...
  if (!styleSheet.cache[id]) {
    if (!styleSheet.inserted[id]) {
      ...
      Object.keys(mediaBag).forEach(function (expr) {
        ...
        var result = plugins.media.apply({ id: id, expr: expr, css: css });
        styleSheet.insert('@media ' + result.expr + ' { ' + result.css.join('\n') + ' }');
      });
      styleSheet.inserted[id] = true;
    }

    styleSheet.cache[id] = _extends({ bag: styleBag, id: id, label: label }, Object.keys(mediaBag).length > 0 ? { media: mediaBag } : {});
  }
  return _defineProperty({}, 'data-css-' + id, label);
}
...
function insertMediaRule(_ref3) {
  var id = _ref3.id;
  var expr = _ref3.expr;
  ...
  styleSheet.cache[id] = { expr: expr, rule: rule, style: style, id: id, label: label };
}
  ...
  if (isRule(rule)) {
    ...
    if (isMerged(id)) {
      var _ret4 = function () {
        ...
        var label = hasLabels ? '*mq [' + spec.label + ']' : '';

        if (!styleSheet.cache[newId]) {
          var css = Object.keys(bag).map(function (type) {
            return cssrule(newId, type, bag[type]);
          });
          insertMediaRule({ id: newId, expr: expr, rule: rule, label: label, css: css });
        }
      ...
      var _label = hasLabels ? '*mq ' + (spec.style.label || '*') : '';

      if (!styleSheet.cache[_newId]) {
        var css = [cssrule(_newId, spec.type, spec.style)];
        insertMediaRule({ id: _newId, expr: expr, rule: rule, label: _label, css: css });
      }
    ...
    // a plain style 
    var _style4 = rule;
    var _newId2 = styleHash(expr, _style4);
    var _label2 = hasLabels ? '*mq ' + (_style4.label || '*') : '';
    if (!styleSheet.cache[_newId2]) {
      var _css = [cssrule(_newId2, '_', _style4)];
      insertMediaRule({ id: _newId2, expr: expr, style: rule, label: _label2, css: _css });
    }
...
function updateMediaQueryLabels() {
  Object.keys(styleSheet.cache).forEach(function (id) {
    var expr = styleSheet.cache[id].expr;
...
function fontFace(font) {
  ...
  if (!styleSheet.cache[id]) {
    styleSheet.cache[id] = { id: id, family: font.fontFamily, font: font };
    // todo - crossbrowser 
    insertRule('@font-face { ' + (0, _CSSPropertyOperations.createMarkupForStyles)(font) + '}', id);
    // if(!styleSheet.inserted[id]) {
    //   styleSheet.insert()  
    //   styleSheet.inserted[id] = true
    // }
...
function keyframes(name, kfs) {
  ...
  if (!styleSheet.cache[id]) {
    styleSheet.cache[id] = { id: id, name: name, kfs: kfs };

    if (!styleSheet.inserted[id]) {
      (function () {
        styleSheet.inserted[id] = true;
        var inner = Object.keys(kfs).map(function (kf) {
          var result = plugins.keyframes.apply({ id: id, name: kf, style: kfs[kf] });
          return result.name + ' { ' + (0, _CSSPropertyOperations.createMarkupForStyles)(result.style) + '}';
        }).join('\n');

        ['-webkit-', '-moz-', '-o-', ''].forEach(function (prefix) {
          return styleSheet.insert('@' + prefix + 'keyframes ' + (name + '_' + id) + ' { ' + inner + '}');
        });
      })();
    }
  }
...
function cssFor() {
  ...
  var css = styleSheet.rules().map(function (_ref6) {
    var cssText = _ref6.cssText;
...
function makeStyleTag() {
  var name = arguments.length <= 0 || arguments[0] === undefined ? '_css_' : arguments[0];
...
var StyleSheet = exports.StyleSheet = function () {
  function StyleSheet(_ref) {
    ...
    var name = _ref$name === undefined ? '_css_' + sheetCounter++ : _ref$name;
    var _ref$speedy = _ref.speedy;
    var speedy = _ref$speedy === undefined ? !isDev && !isTest : _ref$speedy;
    var _ref$length = _ref.length;
    var length = _ref$length === undefined ? isBrowser && oldIE ? 4000 : 65000 : _ref$length;
  ...
  _createClass(StyleSheet, [{
    ...
    value: function insert(rule) {
      ...
      if (this.tags.length > 0 && (_context = this.tags, last).call(_context).styleSheet) {
        var _context2;
    ...
    value: function rules() {
      ...
      return this.tags.reduce(function (arr, tag) {
        return arr.concat(Array.from(sheetForTag(tag).cssRules));
      }, []);
    }
  }]);
