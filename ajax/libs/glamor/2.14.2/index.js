...
var react_lib_CSSPropertyOperations = require('./CSSPropertyOperations.js');
...
var StyleSheet = function () {
  function StyleSheet() {
    ...
    var speedy = _ref$speedy === undefined ? !isDev$1 && !isTest$1 : _ref$speedy;
    var _ref$maxLength = _ref.maxLength;
    var maxLength = _ref$maxLength === undefined ? isBrowser$1 && oldIE ? 4000 : 65000 : _ref$maxLength;
...
var autoprefix = gate(true);

var isDev$2 = function (x) {
  return x === 'development' || !x;
}("production");

var PluginSet = function () {
  function PluginSet() {
    classCallCheck(this, PluginSet);
    ...
    value: function add() {
      ...
      fns.forEach(function (fn) {
        if (_this.fns.indexOf(fn) >= 0) {
          ...
          _this.fns = [fn].concat(toConsumableArray(_this.fns));
        }
      });
    }
  }, {
...
function fallbacks(node) {
  ...
  if (hasArray) {
    var _ret = function () {
      var style = node.style;
      var rest = objectWithoutProperties(node, ['style']);
...
function prefixes(_ref) {
  ...
  var rest = objectWithoutProperties(_ref, ['style']);
...
var plugins = styleSheet.plugins = new PluginSet(fallbacks, bug20fix, prefixes);
...
function flatten() {
  ...
  for (var i = 0; i < els.length; i++) {
    if (Array.isArray(els[i])) arr = arr.concat(flatten.apply(undefined, toConsumableArray(els[i])));else arr = arr.concat(els[i]);
  }
  return arr;
}
...
function hashify() {
  ...
  return doHash(objs.map(function (x) {
...
function extractStyles() {
  ...
  rules.forEach(function (rule) {
    ...
    switch (rule.type) {
      ...
      case 'pseudo':
        return deepMergeStyles(exprs, { _: defineProperty({}, '%%%' + rule.selector, rule.style) });
      case 'select':
        return deepMergeStyles(exprs, { _: defineProperty({}, '^^^' + rule.selector, rule.style) });
      case 'parent':
        return deepMergeStyles(exprs, { _: defineProperty({}, '***' + rule.selector, rule.style) });
      ...
      case 'media':
        return deepMergeStyles(exprs, defineProperty({}, rule.expr, extractStyles(rule.rules)._));
...
function toCSS(_ref4) {
  ...
  return result.selector + ' { ' + react_lib_CSSPropertyOperations.createMarkupForStyles(result.style) + ' }';
}
...
function ruleToCSS(spec) {
  ...
  // plugins here 
  var _ = ast._;
  var exprs = objectWithoutProperties(ast, ['_']);
  ...
  if (!inserted[spec.id]) {
    (function () {
      var inner = Object.keys(spec.keyframes).map(function (kf) {
        ...
        return result.name + ' { ' + react_lib_CSSPropertyOperations.createMarkupForStyles(result.style) + '}';
      }).join('\n');
...
function insertFontFace(spec) {
  if (!inserted[spec.id]) {
    styleSheet.insert('@font-face { ' + react_lib_CSSPropertyOperations.createMarkupForStyles(spec.font) + '}');
    inserted[spec.id] = true;
  }
}
...
function toRule(spec) {
  register(spec);
  insert(spec);
  return defineProperty({}, 'data-css-' + spec.id, hasLabels ? spec.label || '' : '');
}
...
},{"./CSSPropertyOperations.js":1}]},{},[2])(2)
});