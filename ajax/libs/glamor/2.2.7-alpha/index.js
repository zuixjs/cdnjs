...
function selector(id, type) {
  ...
  if (isFullSelector) {
    suffix = cssType.split(',').map(function (x) {
...
function merge() {
          ...
          if (rule) {
            ...
            if (cache[iid].bag) {
              (function () {
                ...
                Object.keys(bag).forEach(function (type) {
                  mediaBag[expr].bag[type] = _extends({}, mediaBag[expr].bag[type] || {}, bag[type]);
                });
              })();
            } else {
              ...
              mediaBag[expr].bag[type] = _extends({}, mediaBag[expr].bag[type] || {}, _style);
            }
          } else {
            mediaBag[expr].bag._ = _extends({}, mediaBag[expr].bag._ || {}, style);
          }
  ...
  if (!cache[id]) {
    cache[id] = _extends({ bag: styleBag, id: id, label: label }, Object.keys(mediaBag) > 0 ? { media: mediaBag } : {});
...
function cssFor() {
  ...
  var css = [].concat(_toConsumableArray(styleSheet.cssRules)).map(function (_ref6) {
...
},{"./CSSPropertyOperations":1,"./autoprefix":2,"./hash":3}]},{},[4])(4)
});