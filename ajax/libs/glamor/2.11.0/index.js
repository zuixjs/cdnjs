...
var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); /**** stylesheet ****/
...
var plugins = exports.plugins = new Plugins(_plugins.prefixes, _plugins.fallbacks);
plugins.media = new Plugins(); // neat! media, font-face, keyframes
plugins.fontFace = new Plugins();
plugins.keyframes = new Plugins(_plugins.prefixes);
//////
...
function simulate() {
  ...
  for (var _len3 = arguments.length, pseudos = Array(_len3), _key3 = 0; _key3 < _len3; _key3++) {
    pseudos[_key3] = arguments[_key3];
  }
...
function merge() {
  ...
  if (!styleSheet.cache[id]) {
    ...
    Object.keys(mediaBag).forEach(function (expr) {
      ...
      var result = plugins.media.apply({ id: id, media: true, expr: expr, css: css });
  ...
  if (isRule(rule)) {
    ...
    if (styleSheet.cache[id].bag) {
      var _ret4 = function () {
        ...
        var bag = styleSheet.cache[id].bag;

        var newId = (0, _hash2.default)(expr + id).toString(36);
        var label = hasLabels ? '*mq [' + styleSheet.cache[id].label + ']' : '';

        if (!styleSheet.cache[newId]) {
          var cssRules = Object.keys(bag).map(function (type) {
            return cssrule(newId, type, bag[type]);
          });
          var result = plugins.media.apply({ id: newId, expr: expr, css: cssRules });
          styleSheet.insert('@media ' + result.expr + ' { ' + result.css.join('\n') + ' }');
          styleSheet.cache[newId] = { expr: expr, rule: rule, id: newId, label: label };
        }
    ...
    } else if (styleSheet.cache[id].expr) {
      // media rule
      throw new Error('cannot apply a media rule onto another');
    } else {
      // simple rule
      var _newId = (0, _hash2.default)(expr + id).toString(36);
      var _label = hasLabels ? '*mq ' + (styleSheet.cache[id].style.label || '*') : '';

      if (!styleSheet.cache[_newId]) {
        var css = cssrule(_newId, styleSheet.cache[id].type, styleSheet.cache[id].style);
        var result = plugins.media.apply({ id: _newId, expr: expr, css: [css] });
        styleSheet.insert('@media ' + result.expr + ' { ' + result.css.join('\n') + ' }');
        styleSheet.cache[_newId] = { expr: expr, rule: rule, id: _newId, label: _label };
      }
    ...
    if (!styleSheet.cache[_newId2]) {
      ...
      var _result = plugins.media.apply({ id: _newId2, expr: expr, css: [_css] });
      styleSheet.insert('@media ' + _result.expr + ' { ' + _result.css.join('\n') + ' }');
      styleSheet.cache[_newId2] = { expr: expr, style: _style3, id: _newId2, label: _label2 };
    }
...
function cssFor() {
  ...
  var css = [].concat(_toConsumableArray(styleSheet.sheet.cssRules)).map(function (_ref5) {
...
var StyleSheet = exports.StyleSheet = function () {
  function StyleSheet(_ref) {
    ...
    var name = _ref$name === undefined ? '_css_' + tagCounter++ : _ref$name;
  ...
  _createClass(StyleSheet, [{
    ...
    value: function inject() {
      ...
      if (isBrowser) {
        ...
        this.sheet = [].concat(_toConsumableArray(document.styleSheets)).filter(function (x) {
          return x.ownerNode === _this.tag;
        })[0];
      } else {
        ...
        this.sheet = {
          ...
          deleteRule: function deleteRule(index) {
            _this.sheet.cssRules = [].concat(_toConsumableArray(_this.sheet.cssRules.slice(0, index)), _toConsumableArray(_this.sheet.cssRules.slice(index + 1)));
          },
          insertRule: function insertRule(rule) {
            var index = arguments.length <= 1 || arguments[1] === undefined ? _this.sheet.cssRules.length : arguments[1];

            // enough 'spec compliance' to be able to extract the rules later  
            // in other words, just the cssText field 
            _this.sheet.cssRules = [].concat(_toConsumableArray(_this.sheet.cssRules.slice(0, index)), [{ cssText: rule }], _toConsumableArray(_this.sheet.cssRules.slice(index)));
          }
        };
      }
      this.injected = true;
    ...
    key: '_insert',
    value: function _insert(rule, index) {
    ...
    value: function insert(rule) {
      ...
      var index = arguments.length <= 1 || arguments[1] === undefined ? this.sheet.cssRules.length : arguments[1];
      ...
      if (this.tag && this.tag.styleSheet) {
        ...
        if (isBrowser) {
          if (this.speedy && this.sheet.insertRule) {
            ...
            var refNode = this.tag.childNodes[index]; // not index - 1 as expected, because we have a blank leading node 
