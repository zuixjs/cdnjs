...
var autoprefix = (0, _autoprefix2.default)(true);
...
styleSheet = void 0; // reference to the styleSheet object, either native on browser / polyfilled on server 
...
function selector(id, type) {
  ...
  var cssType = type === '_' ? '' : type[0] === '$' ? type.slice(1) : type[0] === '%' ? type.slice(1) : ':' + type;
  var result = void 0;
...
function media(expr) {
  ...
  if (isRule(rule)) {
    ...
    if (cache[id].bag) {
      ...
      var _label = hasLabels ? '*mq ' + (cache[id].style.label || '*') : '';
