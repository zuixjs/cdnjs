...
var BaseStyledComponent = function (_PureComponent) {
  ...
  BaseStyledComponent.prototype.renderInner = function renderInner(theme) {
    ...
    for (key in this.props) {
      if (key === 'forwardedClass' || key === 'as') continue;else if (process.env.NODE_ENV !== 'production' && key === 'innerRef') {
        warnInnerRef();
...
function createGlobalStyle(strings) {
  ...
  var GlobalStyleComponent = function (_React$PureComponent) {
    ...
    GlobalStyleComponent.prototype.componentWillUnmount = function componentWillUnmount() {
      ...
      if (count === 0) globalStyle.removeStyles(this.styleSheet);else if (process.env.NODE_ENV !== 'production' && IS_BROWSER && count > 1) {
