...
var flatten$1 = function flatten$$1(chunks, executionContext) {
  /* Fall back to old flattener for non-rule-set chunks */
  if (!isRuleSet(chunks)) {
    return flatten(chunks, executionContext);
  }
  ...
  return chunks.reduce(function (ruleSet, chunk) {
    ...
    var newChunk = chunk.reduce(function (rules, rule) {
      ...
      if (isRuleSet(rule)) {
        ...
        appendChunks = [].concat(appendChunks, flatten$$1(rule, executionContext));
        return rules;
      }

      /* Stringify unexpected array */
      if (Array.isArray(rule)) {
        return [].concat(rules, flatten(rule, executionContext));
      }
      ...
      if (typeof rule === 'function') {
        if (executionContext) {
          ...
          if (isRuleSet(res)) {
            appendChunks = [].concat(appendChunks, flatten$$1(res, executionContext));
            return rules;
          }

          /* Flatten non-ruleset values */
          return [].concat(rules, flatten$$1([res], executionContext));
        } else {
...
var css = (function (interpolations) {
  return flatten$1(interpolations);
});
...
var warnInjectGlobalDeprecated = void 0;
if (process.env.NODE_ENV !== 'production') {
  warnInjectGlobalDeprecated = once(function () {
    // eslint-disable-next-line no-console
    console.error('Notice: The "injectGlobal" API will be removed in the upcoming v4.0 release. Use "createGlobalStyle" instead. You can find more information here: https://github.com/styled-components/styled-components/issues/1333');
  });
}
...
var ComponentStyle = _ComponentStyle(generateAlphabeticName, flatten$1, stringifyRules);
...
//# sourceMappingURL=styled-components-no-parser.esm.js.map
