...
var makeStyleTag = function makeStyleTag(target, tagEl, insertBefore) {
  ...
  el.setAttribute(SC_VERSION_ATTR, "4.0.3-0");
...
var wrapAsHtmlTag = function wrapAsHtmlTag(css, names) {
  return function (additionalAttrs) {
    var nonce = getNonce();
    var attrs = [nonce && 'nonce="' + nonce + '"', SC_ATTR + '="' + stringifyNames(names) + '"', SC_VERSION_ATTR + '="' + "4.0.3-0" + '"', additionalAttrs];
...
var wrapAsElement = function wrapAsElement(css, names) {
  return function () {
    ...
    var props = (_props = {}, _props[SC_ATTR] = stringifyNames(names), _props[SC_VERSION_ATTR] = "4.0.3-0", _props);
...
var makeSpeedyTag = function makeSpeedyTag(el, getImportRuleTag) {
  ...
  var insertSourceMap = function insertSourceMap() {}; // not implemented
  ...
  return {
    ...
    insertSourceMap: insertSourceMap,
...
var makeBrowserTag = function makeBrowserTag(el, getImportRuleTag) {
  ...
  var insertSourceMap = function insertSourceMap(sourceMap) {
  ...
  var cssWithSourceMap = function cssWithSourceMap() {
    if (sourceMapContent) {
...
var StyleSheet = function () {
  ...
  StyleSheet.prototype.rehydrate = function rehydrate() {
    ...
    var nodes = document.querySelectorAll('style[' + SC_ATTR + '][' + SC_VERSION_ATTR + '="' + "4.0.3-0" + '"]');
  ...
  StyleSheet.prototype.createTagForId = function createTagForId(id, lastTag) {
    this.capacity = MAX_SIZE;
    var tag = this.makeTag(lastTag);
    this.tags.push(tag);
    this.tagMap[id] = tag;
    return tag;
  };

  StyleSheet.prototype.getPrevUnsealedTagForId = function getPrevUnsealedTagForId(id) {
  ...
  StyleSheet.prototype.getTagForId = function getTagForId(id) {
    ...
    var prev = this.getPrevUnsealedTagForId(id);
    if (prev) {
      return prev;
    }
  ...
  StyleSheet.prototype.deferredInject = function deferredInject(id, cssRules, sourceMap) {
    var _this2 = this;
    ...
    var tag = function () {
      if (sourceMap && !_this2.getPrevUnsealedTagForId(id)) {
        return _this2.createTagForId(id);
      }
      return _this2.getTagForId(id);
    }();
    tag.insertMarker(id);
  ...
  StyleSheet.prototype.inject = function inject(id, cssRules, name, sourceMap) {
    var _this3 = this;
    ...
    var tag = function () {
      if (sourceMap && !_this3.getPrevUnsealedTagForId(id)) {
        return _this3.createTagForId(id);
      }
      return _this3.getTagForId(id);
    }();
...
var ComponentStyle = function () {
  function ComponentStyle(rules, attrs, componentId, sourceMap) {
  ...
  ComponentStyle.prototype.generateAndInjectStyles = function generateAndInjectStyles(executionContext, styleSheet) {
    ...
    if (!styleSheet.hasNameForId(componentId, name)) {
      styleSheet.inject(this.componentId, stringifyRules(flatCSS, '.' + name, undefined, componentId), name, this.sourceMap);
    }
...
var StyledComponent = function (_Component) {
  ...
  StyledComponent.prototype.renderInner = function renderInner(theme) {
    ...
    for (key in this.props) {
      ...
      if (key === 'forwardedClass' || key === 'as') continue;else if (key === 'forwardedRef') propsForElement.ref = this.props[key];else if (!isTargetTag || validAttr(key)) {
...
function createStyledComponent(target, options, rules) {
  ...
  // $FlowFixMe
  target.componentStyle.rules.concat(rules) : rules, finalAttrs, styledComponentId, sourceMap);
...
function styledComponentsMacro(_ref) {
  ...
  var imports = t.importDeclaration([], t.stringLiteral('styled-components'));
  ...
  var stateWithOpts = _extends({}, state, { opts: config });
