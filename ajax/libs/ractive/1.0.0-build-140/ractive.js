/*
	...
	Build: 616b18e0d0603535af72d992c6e67ba381d0888c
	Date: Wed Feb 28 2018 06:51:38 GMT+0000 (UTC)
...
ModelBase__proto__.getValueChildren = function getValueChildren (value) {
  ...
  if (computed) {
    children.push.apply(
      children,
      keys(computed).map(function (k) { return this$1.joinKey(k); })
    );
  }
...
ModelBase__proto__.has = function has (key) {
  ...
  if (computed) {
    keys(computed).forEach(function (k) {
      if (computed[k].pattern && computed[k].pattern.test(this$1.getKeypath()))
        { return true; }
    });
  }
...
var Model = (function (ModelBase) {
  ...
  Model__proto__.teardown = function teardown$3 () {
    ...
    if (this.computed)
      { keys(this.computed).forEach(function (k) { return this$1.computed[k].teardown(); }); }
  };
...
var RootModel = (function (Model) {
  ...
  RootModel__proto__.getHelpers = function getHelpers () {
    if (!this.helpers)
      { this.helpers = new SharedModel(this.ractive.helpers, "helpers"); }
    return this.helpers;
  };
...
var proto$3 = (MustacheContainer.prototype = Object.create(
...
if (win && !win.Ractive) {
  ...
  warn(
    "Ractive already appears to be loaded while loading 1.0.0-edge."
  );
}
