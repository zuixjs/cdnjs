/*
	Ractive.js v0.9.14-edge
	Build: dc6d55d439fad8a3442fa9b98bfabae1e2498e67
	Date: Wed Feb 28 2018 18:42:12 GMT+0000 (UTC)
...
if (hasConsole) {
  var welcomeIntro = [
    "%cRactive.js %c0.9.14-edge %cin debug mode, %cmore...",
    "color: rgb(114, 157, 52); font-weight: normal;",
    "color: rgb(85, 85, 85); font-weight: normal;",
    "color: rgb(85, 85, 85); font-weight: normal;",
    "color: rgb(82, 140, 224); font-weight: normal; text-decoration: underline;"
  ];
  var welcomeMessage = "You're running Ractive 0.9.14-edge in debug mode - messages will be printed to the console to help you fix problems and optimise your application.\n\nTo disable debug mode, add this line at the start of your app:\n  Ractive.DEBUG = false;\n\nTo disable debug mode when your app is minified, add this snippet:\n  Ractive.DEBUG = /unminified/.test(function(){/*unminified*/});\n\nGet help and support:\n  http://ractive.js.org\n  http://stackoverflow.com/questions/tagged/ractivejs\n  http://groups.google.com/forum/#!forum/ractive-js\n  http://twitter.com/ractivejs\n\nFound a bug? Raise an issue:\n  https://github.com/ractivejs/ractive/issues\n\n";
    ...
    var loop = function ( prop ) {
      if (hasOwn(from, prop)) {
        if (hasOwn(to, prop)) {
          ...
          interpolators[prop] =
            interpolate(from[prop], to[prop]) || (function () { return to[prop]; });
        } else {
...
function findAlias(name, fragment) {
  while (fragment) {
    ...
    if (z && z[name]) {
      var aliases = (fragment.owner.iterations ? fragment.owner : fragment)
        .owner.template.z;
...
var LinkModel = (function (ModelBase) {
  ...
  LinkModel__proto__.getKeypathModel = function getKeypathModel (ractive) {
    ...
    if (ractive && ractive !== this.root.ractive)
      { return this.keypathModel.getChild(ractive); }
  ...
  if (isObject(keypath)) {
    var loop = function ( k ) {
      if (hasOwn(keypath, k)) {
        sets.push.apply(
          sets,
          gather(ractive, k, null, isolated).map(function (m) { return [m, keypath[k], k]; })
        );
      }
    };
    ...
    sets.push.apply(
      sets,
      gather(ractive, keypath, null, isolated).map(function (m) { return [m, value, keypath]; })
    );
  }
...
function getComparator(option) {
  ...
  if (isString(option)) {
    return (
      comparators[option] || (comparators[option] = function (thing) { return thing[option]; })
    );
  }
...
function Ractive$animate(keypath, to, options) {
  if (isObjectType(keypath)) {
    ...
    throw new Error(("ractive.animate(...) no longer supports objects. Instead of ractive.animate({\n  " + (keys$$1.map(function (key) { return ("'" + key + "': " + (keypath[key])); }).join("\n  ")) + "\n}, {...}), do\n\n" + (keys$$1
      .map(function (key) { return ("ractive.animate('" + key + "', " + (keypath[key]) + ", {...});"); })
      .join("\n")) + "\n"));
  }
...
function fireEvent(ractive, eventName, context, args) {
  ...
  var eventNames = ractive._nsSubs
...
function Ractive$findComponent(selector, options) {
  ...
  if (options.remote) {
    ...
    for (var i = 0; i < this._children.length; i++) {
      ...
      if (this$1._children[i].name === selector)
...
function findEvent(el, name) {
  return (
    el.attributes &&
    el.attributes.find(
      function (a) { return a.template.t === EVENT && ~a.template.n.indexOf(name); }
    )
  );
}
    ...
    if (!this.changed.length) {
      ...
      if (this.recursive) {
        ...
        var ok = this.baseModel.isRoot
          ...
          : this.changed.map(
              function (keys$$1) { return this$1.baseKeypath + "." + keys$$1.map(escapeKey).join("."); }
            );
...
var cleanCss = function(css, callback, additionalReplaceRules) {
  ...
  css = css
    .replace(escape, function (match) { return ("\u0000" + (values.push(match) - 1)); })
    .replace(remove, "");
...
var getLowestIndex = function(haystack, needles) {
  return haystack.search(
    regExpCache[needles.join()] ||
      (regExpCache[needles.join()] = new RegExp(
...
var codeToChar = codePointSupport
...
function readReference(parser) {
  ...
  if (prefix === "@") {
    if (!specials.test(name$$1)) {
      ...
      (~name$$1.indexOf("event") || ~name$$1.indexOf("node")) &&
      !parser.inEvent
    ) {
      parser.error(
      ...
      case OBJECT_LITERAL:
        if (node.m && hasSpread(node.m)) {
          ...
          return (
            "{" +
            (node.m
              ? node.m.map(function (n) { return ((n.k) + ":" + (stringify(n.v))); }).join(",")
              : "") +
            "}"
          );
        }
...
var handlebarsBlockPattern = new RegExp(
  ...
  while (i--) {
    ...
    if (item.f) {
      ...
      if (!preserveWhitespaceInsideFragment) {
        ...
        if (
          !nextItem ||
          (isString(nextItem) && leadingWhitespace$1.test(nextItem))
        ) {
...
var StandardParser = Parser.extend({
  init: function init(str, options) {
    ...
    defaultInterpolate.forEach(
      function (t) { return (this$1.interpolate[t] =
...
var isBlacklisted = makeObj(
  defaultKeys.concat(registries.map(function (r) { return r.name; }), [
    "on",
    "observe",
    "attributes",
    "cssData"
...
var ComputationChild = (function (Model) {
  ...
  ComputationChild__proto__.get = function get (shouldCapture, opts) {
    ...
    return (opts && "unwrap" in opts ? opts.unwrap !== false : shouldCapture) &&
      this.wrapper
      ? this.wrapperValue
      : this.value;
  };
  ...
  ReferenceExpressionProxy__proto__.teardown = function teardown () {
    ...
    if (this.members) {
      this.members.forEach(
        function (m) { return m && m.unregister && m.unregister(this$1.intermediary); }
      );
    }
  };
  ...
  while (i < keys$$1.length) {
    if (keys$$1[i] in style) {
      ...
      style.setProperty(
        keys$$1[i],
        safe,
        safe.length !== props[keys$$1[i]].length ? "important" : ""
      );
    }
    i++;
  }
...
function Ractive$teardown() {
  ...
  return teardown$1(
    this,
    function () { return (this$1.fragment.rendered ? this$1.unrender() : Promise.resolve()); }
  );
}
...
function subscribe(instance, options, type) {
  var subs = (instance.constructor[("_" + type)] || []).concat(
...
function handleAttributes(ractive) {
  ...
  if (attributes && component) {
    ...
    attributes.required.forEach(function (p) {
      if (!~props.indexOf(p)) {
        warnIfDebug(
          ("Component '" + (component.name) + "' requires attribute '" + p + "' to be provided")
        );
      }
    });
...
var Element = (function (ContainerItem) {
  ...
  Element__proto__.render = function render$3 (target, occupants) {
    ...
    if (this.up.cssIds) {
      node.setAttribute(
        "data-ractive-css",
        this.up.cssIds.map(function (x) { return ("{" + x + "}"); }).join(" ")
      );
    }
...
var proto$4 = (MustacheContainer.prototype = Object.create(
...
function createMapping(item) {
  ...
  if (template.length === 1 && template[0].t === INTERPOLATOR) {
    ...
    if (!template[0].s) {
      ...
      warnIfDebug(
        ("Cannot copy non-computed object value from static mapping '" + (item.name) + "'")
      );
  ...
  resetTemplate: function resetTemplate() {
    ...
    if (this.fn) {
      ...
      if (isFunction(this.proxy.render))
        { runloop.scheduleTask(function () { return this$1.proxy.render(); }); }
  ...
  if (isArray(fn.attributes)) {
    ...
    if (isArray(template.m)) {
      ...
      template.p[extras] = template.m = attrs.filter(
        function (a) { return !~fn.attributes.indexOf(a.n); }
      );
...
Transition__proto__.animateStyle = function animateStyle (style, value, options) {
  ...
  return new Promise(function (fulfil) {
    ...
    while (i--) {
      ...
      if (!(name in this$1.originals))
        { this$1.originals[name] = this$1.node.style[name]; }
...
Fragment__proto__.findAllComponents = function findAllComponents (name, options) {
  if (this.items) {
    this.items.forEach(
      function (i) { return i.findAllComponents && i.findAllComponents(name, options); }
    );
  }
};
...
function Ractive$toggle(keypath, options) {
  ...
  return set(
    gather(this, keypath, null, options && options.isolated).map(function (m) { return [
      m,
      !m.get()
    ]; }),
    options
...
function Ractive$toCSS() {
  ...
  var uniqueCssIds = keys(
    cssIds.reduce(function (ids, id) { return ((ids[id] = true), ids); }, {})
  );
...
if (win && !win.Ractive) {
  ...
  warn(
    "Ractive already appears to be loaded while loading 0.9.14-edge."
  );
}
