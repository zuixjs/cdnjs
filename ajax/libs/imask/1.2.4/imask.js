...
var Masked = function () {
  ...
  Masked.prototype.appendWithTail = function appendWithTail(str, tail) {
    ...
    for (var ci = 0; ci < str.length; ++ci) {
      ...
      var appended = this._append(ch, { input: true });
...
var MaskedNumber = function (_Masked) {
  ...
  MaskedNumber.prototype._update = function _update(opts) {
    opts.postFormat = _extends({}, this.postFormat, opts.postFormat);
...
var MaskedPattern = function (_Masked) {
  ...
  MaskedPattern.prototype._update = function _update(opts) {
    opts.placeholder = _extends({}, this.placeholder, opts.placeholder);
  ...
  MaskedPattern.prototype._appendChunks = function _appendChunks(chunks) {
    ...
    for (var ci = 0; ci < chunks.length; ++ci) {
      ...
      if (this._append.apply(this, [input].concat(args)) === false) return false;
    }
    return true;
  };
...
var MaskedDynamic = function (_Masked) {
  ...
  function MaskedDynamic(opts) {
    ...
    _this.currentMasked = null;
    return _this;
  }

  MaskedDynamic.prototype._update = function _update(opts) {
    _Masked.prototype._update.call(this, opts);
    this._compiledMasks = Array.isArray(opts.mask) ? opts.mask.map(function (m) {
      return createMask(m);
    }) : [];
  };
  ...
  MaskedDynamic.prototype._append = function _append(str) {
    ...
    this.currentMasked = this.doDispatch.apply(this, [str].concat(args));

    // update current mask
    this.currentMasked.rawInputValue = inputValue;
    return this.value.length - oldValueLength + (_currentMasked = this.currentMasked)._append.apply(_currentMasked, [str].concat(args));
  };

  MaskedDynamic.prototype.appendWithTail = function appendWithTail(str, tail) {
    // TODO seems strange
    // call append to do dispatch and get offset
    var offset = !str ? this._append('') : 0;
    return offset + _Masked.prototype.appendWithTail.call(this, str, tail);
  };
  ...
  MaskedDynamic.prototype.clone = function clone() {
    ...
    m._compiledMasks.forEach(function (cm, i) {
      return cm._value = _this2._compiledMasks[i].value;
    });
    return m;
  };
  ...
  MaskedDynamic.prototype.reset = function reset() {
    ...
    this._compiledMasks.forEach(function (cm) {
      return cm.reset();
    });
  };
  ...
  MaskedDynamic.prototype.clear = function clear() {
    var _currentMasked2;
    ...
    if (this.currentMasked) (_currentMasked2 = this.currentMasked).clear.apply(_currentMasked2, args);
    this._compiledMasks.forEach(function (cm) {
      return cm.clear.apply(cm, args);
    });
  };
  ...
  MaskedDynamic.prototype.extractInput = function extractInput() {
    ...
    return this.currentMasked ? (_currentMasked3 = this.currentMasked).extractInput.apply(_currentMasked3, arguments) : '';
  };
...
MaskedDynamic.DEFAULTS = {
  dispatch: function dispatch(appended, masked, flags) {
    ...
    // update all
    masked._compiledMasks.forEach(function (cm) {
      cm.rawInputValue = inputValue;
      cm._append(appended, flags);
    });

    // pop masks with longer values first
    var inputs = masked._compiledMasks.map(function (cm, index) {
